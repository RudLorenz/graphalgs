...
int main(int argc, char**argv)
{
    ...
    const auto file_option = app.add_option("-f, --file", filename, "Input file with adjacency matrix")
        ->check(CLI::ExistingFile)
        ->group("Input");
    const auto interactive_option = app.add_flag("-i, --interactive", interactive_mode, "Input graph in interactive mode")
        ->group("Input");
    ...
    const auto file_out_flag = app.add_option("-o, --output", output_file, "Output graph in given a file")
        ->group("Output");

    const auto json_out_flag = app.add_flag("-j, --json", "Output graph as a json")
        ->group("Output")
        ->needs(file_out_flag);

    const auto matrix_out_flag = app.add_flag("-m, --matrix", "Output graph as a matrix")
        ->group("Output")
        ->needs(file_out_flag);
    ...
    if (hamilton_mode)
    {
        const auto result = glp.findHamiltonCycles();
    ...
    if (art_points_mode)
    {
        const auto ap_points = glp.getArticulationPoints(groups);
        if (ap_points.empty()) {
